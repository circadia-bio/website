...
(function(){
  ...
  function makeCard(p, isClone) {
    ...
    // Zenodo/CITATION.cff footer strip — only for tracked packages.
    // p.zenodoTracked / p.version / p.doiUrl / p.releasedDate / p.citationUrl
    // are injected by generate_zenodo_badges.py.
    if (p.zenodoTracked) {
      ...
      if (p.version) {
        ...
        if (p.citationUrl) {
          var citeOnly = document.createElement('a');
          citeOnly.className = 'pc-cite-only';
          citeOnly.href = p.citationUrl;
          citeOnly.target = '_blank';
          citeOnly.rel = 'noopener';
          citeOnly.textContent = 'Cite';
          meta.appendChild(citeOnly);
        }
      }
